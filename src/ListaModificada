...
#ifndef LISTAMODIFICADA_H_
#define LISTAMODIFICADA_H_

#include "NodoModificado.h"
...
template<class T> class Lista {
    ...
    public:
         ...
         *
         */
        void agregar(T elemento, unsigned int coordenada);
    ...
    private:
         ...
         */
        Nodo<T>* obtenerNodo(unsigned int coordenada); // NOTA: primitiva PRIVADA

        Nodo<T>* obtenerNodoAnteriorALaCoordenada( unsigned int coordenada );
};
...
template<class T> void Lista<T>::agregar(T elemento, unsigned int coordenada) {

    if ((coordenada > 0) && (coordenada <= this->limite)) {

        Nodo<T>* nuevo = new Nodo<T>(elemento, coordenada);
...
template<class T>
Nodo<T>* Lista<T>::obtenerNodo(unsigned int coordenada) {
    ...
    while (( i<coordenada) && (i!=0)) {
        ...
        i= actual->obtenerCoordenadaSiguiente();
    }
...
template<class T>
Nodo<T>* Lista<T>::obtenerNodoAnteriorALaCoordenada( unsigned int coordenada){
	Nodo<T>* actual= this->primero;
	this->iniciarCursor();
	while ((this->avanzarCursor()) && (actual->obtenerCoordenada()< coordenada)){
		actual= this->obtenerCursor();
	}

	return actual;
}

#endif /* LISTAMODIFICADA_H_ */