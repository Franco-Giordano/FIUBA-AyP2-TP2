...
void DrawAALine(BMP &Image, int FromX, int FromY, int ToX, int ToY, RGBApixel Color) {
	...
	for (i = LeftI; i <= RightI; i++) {
		for (j = LeftJ; j <= RightJ; j++) {
			...
			if (Temp > MinValue) {
				Image(i, j)->Red = (ebmpBYTE) (unsigned int) ((1.0 - Temp) * ((double) Image(i, j)->Red) + Temp * ((double) Color.Red));
				Image(i, j)->Green = (ebmpBYTE) (unsigned int) ((1.0 - Temp) * ((double) Image(i, j)->Green) + Temp * ((double) Color.Green));
				Image(i, j)->Blue = (ebmpBYTE) (unsigned int) ((1.0 - Temp) * ((double) Image(i, j)->Blue) + Temp * ((double) Color.Blue));
			}
...
void DrawArc(BMP &Image, double CenterX, double CenterY, double Radius, double FromTheta, double ToTheta, RGBApixel Color) {
	double pi = 3.14159265358979;
	...
	for (x = 0; x < Temp.TellWidth(); x++) {
		for (y = 0; y < Temp.TellHeight(); y++) {
			...
			if (InnerRadiusSquared <= TempRadiusSquared && TempRadiusSquared <= OuterRadiusSquared) {
	...
	for (i = 0; i < Downsampled.TellWidth(); i++) {
		for (j = 0; j < Downsampled.TellHeight(); j++) {
			...
			if (DestinationI >= 0 && DestinationI < Image.TellWidth() && DestinationJ >= 0 && DestinationJ < Image.TellHeight()) {
				double alpha = (255.0 - Downsampled(i, j)->Red) / 255.0;
				Image(DestinationI, DestinationJ)->Red = (ebmpBYTE) ((1.0 - alpha) * Image(DestinationI, DestinationJ)->Red + (alpha) * Color.Red);
				Image(DestinationI, DestinationJ)->Green = (ebmpBYTE) ((1.0 - alpha) * Image(DestinationI, DestinationJ)->Green + (alpha) * Color.Green);
				Image(DestinationI, DestinationJ)->Blue = (ebmpBYTE) ((1.0 - alpha) * Image(DestinationI, DestinationJ)->Blue + (alpha) * Color.Blue);
			}
